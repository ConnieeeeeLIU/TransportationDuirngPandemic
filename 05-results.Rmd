...
```{r alluvial, fig.width = 16, fig.height = 12}
library(ggalluvial)
library(dplyr)
state_data <- readr::read_csv("data/clean/State.csv")
...
state_agg$covid <- case_when(year(state_agg$Date) == 2019 & month(state_agg$Date) ==7 ~ "before",
                             year(state_agg$Date) == 2020 & month(state_agg$Date) ==12 ~ "after")
...
state_agg <- dplyr::filter(state_agg, state %in% c( 'NY', 'CA', 'MA') )
...
state_confirmed <- melt(covid, id.vars = c('Province_State'),
                           variable.name='Date',
...
ggplot(state_conf_weekly, aes(x=Date, y=Province_State, height=avg_conf, group=Province_State)) +
  geom_density_ridges(stat="identity", scale=1) +
  ggtitle("Daily new confirmed cases in top states")
```

### Boxplot of Trip numbers for each county in New York
```{r boxplot of New York}
NY <- readr::read_csv("data/clean/county/NY.csv")
NY <- NY[which(colnames(NY) %in% c("Date", "County Name", "Number of Trips <1", "Number of Trips 1-3"))]
NY$short <- NY$`Number of Trips <1` + NY$`Number of Trips 1-3`
NY <- NY[-which(colnames(NY) %in% c("Number of Trips <1", "Number of Trips 1-3"))]

#ggplot(NY, aes(x=reorder(`County Name`, short), y=short)) +
...
NY <- dplyr::filter(NY, `County Name` %in% top_county)
ggplot(NY, aes(x=reorder(`County Name`, short), y=short)) +
  geom_boxplot() +
  coord_flip() +
  labs(x="County name", y="Short Trips") +
  ggtitle("Boxplot of Short trip numbers in each county in New York")
```