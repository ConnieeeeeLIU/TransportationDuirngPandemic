...
In this section, we present graphs using `visdat` package([visdat](https://cran.r-project.org/web/packages/naniar/vignettes/getting-started-w-naniar.html)) to visual the missing patterns of our data sets. We present visualizations corresponding to each dataset to help readers to have a comprehensive impression on all the data sets. 

## Dataset1: Trips by distance
As described in section 2, this data set is about the number of trips by distance. First, we show the missing data visualizations. The horizontal axis displays the column in the data set and the vertical axis represents each observations. According to the label, the gray area indicates the missing value in the data set.
...
```{r n2, fig.height = 3, fig.width = 7, eval = FALSE}
...
transportation_data_part <- transportation_data[,c(2,9:11,14,128,130,132)]
...
```{r covid, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide'}
library(visdat)
