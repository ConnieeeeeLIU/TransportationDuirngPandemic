...
Also, we discover that correctly selecting graph types to make visualizations is very important. Each graph has its own features and we need to ones that are most helpful to our goal. Meanwhile, combining different types of graphs also enhances our understanding and analysis to the whole dataset. Thus in our study, we used ten different graphs, and each one of them provides unique insights and inspires us in its own way. 

## Limitations and future directions

Some analysis is not investigated in so much detail because of time limit. To be more specific, in the national-level number of trips plot, there is a surge on the number of trip around March 2020. But we did not fully explore the reason why that was the case with other supporting data or documents. 
